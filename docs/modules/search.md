...
> อ้างอิง: [../project-overview.md](../architecture/project-overview.md#5-search-philosophy) · [item.md](item.md) · [../domain-model.md](../architecture/domain-model.md)

## 1. Purpose
ให้ผู้ใช้หาของได้ทันทีจากคำค้น พร้อม filter ตาม site/status และเห็นผลลัพธ์ที่บอก "อยู่ไหน/สถานะ/ใครถือ" ครบในตา เป็นเส้นทางหลักในการเข้าถึง item

## 2. User Stories
- ในฐานะผู้ใช้ ฉันต้องการ **พิมพ์คำค้นแล้วเห็นผลทันที** (debounced)
- ฉันต้องการ **กรองตาม site / status**
...
| หน้า | Route | คำอธิบาย |
|------|-------|----------|
| Search | `/w/:wsId/search?q=&siteId=&status=&page=` | ช่องค้นหา + filters + ผลลัพธ์ |
...
- `ResultList`, `ItemCard`, `StatusBadge`, `LocationBreadcrumb`
...
| Site filter | `<Select>` | `?siteId=` | dropdown รายการ site |
| Status filter | `<Select>` | `?status=` | stored / taken_out / missing / disposed |
| Clear filters | button | ลบ params | แสดงเมื่อมี filter active |

- ทุก control อัปเดต URL param โดยตรง (ผ่าน `useSearchParams`)
- Mobile: keyword ในหน้า + ปุ่ม "Filters" → Drawer สำหรับ siteId + status

## 6. API Calls
ผ่าน `src/api/item.api.ts` (search ใช้ list endpoint เดียวกับ item):
- `GET /workspaces/:wsId/items?q=&siteId=&status=&page=&limit=&sort=` → `{ data: Item[], meta: { page, limit, total } }`
- (future) `?tag=`, `?barcode=`, `?qr=`
...
## 9. Form Validation
- ไม่มี validation เข้ม — keyword อิสระ; trim ช่องว่าง; status ต้องเป็นค่าที่อนุญาต; siteId ต้องมีจริง (มิฉะนั้น ignore)
...
StatCard "Taken Out" (dashboard) → /search?status=taken_out
```
...
- ค้นด้วย **tag / barcode / QR** (ตาม [../project-overview.md](../architecture/project-overview.md))
- Fuzzy search, highlight คำตรง, ค้นหาในชื่อ container/location
- saved filters, sort หลายแบบ, recent searches
- server-side full-text + ranking

## 17. Definition of Done
- [ ] หน้า Search + Global SearchBar (topbar + dashboard)
- [ ] keyword + filter (site/status) ผูกกับ **URL query params**
