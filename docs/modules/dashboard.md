...
- ฉันต้องการ **shortcut** ไปฟีเจอร์หลัก (Add Item, Sites, Members)

## 3. Screen Description
| หน้า | Route | คำอธิบาย |
|------|-------|----------|
| Dashboard | `/w/:wsId` | search bar เด่น + stat cards + recent activity + quick actions |
ใช้ **AppLayout**
...
- Quick Actions (Add Item / Add Container) เป็น link/ปุ่ม navigate ไม่ใช่ form

## 6. API Calls
ผ่าน `src/api/dashboard.api.ts` (หรือรวม endpoint ที่มีอยู่):
- `GET /workspaces/:wsId/dashboard` → `{ totalItems, stored, takenOut, missing }` (summary)
...
StatCard "Taken Out" คลิก → /search?status=taken_out (filter preset)
Quick Action → หน้าเป้าหมาย (Add Item / Sites / Members)
...
## 13. Empty State
- workspace ใหม่ยังไม่มีของ → stats เป็น 0 + `EmptyState` ใน activity "ยังไม่มีกิจกรรม" + ชวน Add Item / Create Site
...
- แจ้งเตือน missing items

## 17. Definition of Done
- [ ] Dashboard page + AppLayout
- [ ] SearchBar เด่น → ไปหน้า Search
- [ ] Stat cards (total/stored/taken_out/missing) จาก API
