...
|------|-----------------|
| List (Site, Member, Search) | `<SkeletonCard />` × 3–5 รายการ |
...
| Site List | "ยังไม่มี Site" | + เพิ่ม Site |
...
### Filter Pattern
- Filter ผูกกับ URL param: `?status=stored&siteId=xxx`
...
### Desktop
- Table (`<Table />` shadcn/ui) สำหรับ structured data (Members, Sites)
...
| **Form** | Add/Edit Site, Add/Edit Location | ฟอร์มใน dialog (ถ้าไม่ต้องการหน้าเต็ม) |
| **Action** | TakeOut, Return, Move, MarkFound | dialog เฉพาะ action + form เล็ก |
...
| Notifications (future) | right | bell icon |
...
| `taken_out` | `bg-amber-100 text-amber-800` | ถูกยืม |
